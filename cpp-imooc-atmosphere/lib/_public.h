...
public:
    ...
    CCmdStr();  // 构造函数。
    CCmdStr(const std::string &buffer, const char *sepstr, const bool bdelspace = false);

    // 把字符串拆分到m_vCmdStr容器中。
    // buffer：待拆分的字符串。
    // sepstr：buffer中采用的分隔符，注意，sepstr参数的数据类型不是字符，是字符串，如","、" "、"|"、"~!~"。
    // bdelspace：拆分后是否删除字段内容前后的空格，true-删除；false-不删除，缺省不删除。
    void SplitToCmd(const std::string &buffer, const char *sepstr, const bool bdelspace = false);
...
public:
    ...
    bool
    _OpenDir(const char *in_DirName, const char *in_MatchStr, const unsigned int in_MaxCount, const bool bAndChild);
...
class CIniFile {
public:
    std::string m_xmlbuffer; // 存放参数文件全部的内容，由LoadFile方法载入。
